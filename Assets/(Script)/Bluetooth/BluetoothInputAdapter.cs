...
{
    public class BluetoothInputAdapter : MonoBehaviour
    {
        // Forklift
        ...
        private float brakeSpeed = 0.05f;
        ...
        void OnMessageReceived(BluetoothHelper helper)
        {
             ...
             * 第 1 位元組：煞車(0 ~ 9)
             *****************************************************************/
            brakeInt = _receivedBytes[ByteIndex_Brake];
            brakeForPedal = brakeInt / 9f;

            /*
            if (ff > 0)
            {
                brakeOffset += (ff * brakeSpeed);
            }
            else
            {
                brakeOffset = 0f;
