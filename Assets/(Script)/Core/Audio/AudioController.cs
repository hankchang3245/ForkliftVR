...
namespace edu.tnu.dgd.audio
{
	...
	/// <summary>
	/// Background music properties for the AudioController
	/// </summary>
	[Serializable]
	...
	public class AudioController : MonoBehaviour
	{
		...
		[Tooltip("A list of all audio clips attached to the AudioController")]
		...
		/// <summary>
		/// Current and only running instance of the AudioController
		/// </summary>
		public static AudioController instance
		{
			...
			get
			{
				// Check if application is quitting and AudioController is destroyed
				if (!alive)
				{
					Debug.LogWarning(typeof(AudioController) + "' is already destroyed on application quit.");
					return null;
				}
				...
				if (_instance == null)
				{
					...
					if (_instance == null)
					{
						// Lock so can't be used by another thread until release. Useful if two AudioController instances were created simultaneosly
						lock (key)
						{
							//GameObject clone = new GameObject();
							//clone.SetActive(false);
							//clone.AddComponent<AudioController>();

							// Create a new gameobject and add the AudioController component to the gameobject
		...
		/// <summary>
		/// Prevent calling the consructor of the AudioController
		/// </summary>
		...
		/// <summary>
		/// List of audio clips attached to the AudioController
		/// </summary>
		...
		/// <summary>
		/// Is the AudioController processing any background music
		/// </summary>
		...
		// Pool of sound effects managed by the AudioController
		List<SoundEffect> sfxPool = new List<SoundEffect>();
		// Background music managed by the AudioController
		...
		void OnDestroy()
		{
			DestroyAllSound(); // Added By Hank 2021-01-03
			StopAllCoroutines();
			SaveAllPreferences();
		}

		private void DestroyAllSound()
        {
		...
		/// In principle, you shouldn't get to calll the AudioController when your application quits or is quitting.
		...
		/// <summary>
		/// AudioController initialisation tasks
		/// </summary>
		void Initialise()
		{
			...
			// Initialises the sound options used by the AudioController
			...
			if (musicSource == null)
			{
				...
				// If none exists, create one and attach to AudioController
		...
		void Awake()
		{
			if (_instance == null)
			{
				...
				_instance = this;
				// Initialise the AudioController
				Initialise();
			}
		...
		private bool IsMusicAltered()
		{
			...
			if (_musicMixerGroup != null && !string.IsNullOrEmpty(_volumeOfMusicMixer.Trim()))
			{
				...
				// Make it a range of [0 - 1] to suit the music source volume and AudioController volume
		...
		private bool IsSoundFxAltered()
		{
			...
			if (_soundFxMixerGroup != null && !string.IsNullOrEmpty(_volumeOfSFXMixer.Trim()))
			{
				...
				// Make it a range of [0 - 1] to suit the AudioController
				vol = NormaliseVolume(vol);
		...
		private GameObject CreateSoundFx(AudioClip audio_clip, Vector2 location)
		{
			...
			// Parent it to the AudioController until further notice
		...
		private void SetBGMVolume(float volume)
		{
			try
			{
				// Restrict the values to a range of [0 - 1] to suit the AudioController
				volume = Mathf.Clamp01(volume);
				// Assign vol to all music volume variables
				musicSource.volume = currentMusicVol = _musicVolume = volume;

				// Is the AudioController using a master mixer
				if (_musicMixerGroup != null && !string.IsNullOrEmpty(_volumeOfMusicMixer.Trim()))
				{
		...
		/// <summary>
		/// Normalises the volume so that it can be in a range of [0 - 1] to suit the music source volume and the AudioController volume
		/// </summary>
